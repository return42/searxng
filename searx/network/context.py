...
.. code:: python
   ...
   def my_engine():
       ip_ifconfig = context.http_client.request("GET", "https://ifconfig.me/")
       print("ip from ifconfig.me ", ip_ifconfig)
       ip_myip = context.http_client.request("GET", "https://api.myip.com").json()["ip"]
...
class NetworkContext(ABC):
    ...
    method :py:obj:`network.Network.get_context`."""

    _http_client = None

    def __init__(
        self,
        network: 'searx.network.network.Network',
        ...
        self._retries: int = self.network.settings.retries
        # wrap http_client_factory here, so we can forget about this wrapping
        self._http_client_factory = _TimeHTTPClientWrapper.wrap_factory(self)
    ...
    def request(self, *args, **kwargs):
        ...
        return self.call(self.http_client.request, *args, **kwargs)
    ...
    def stream(self, *args, **kwargs):
        ...
        """
        return self.call(self.http_client.stream, *args, **kwargs)
    ...
    @property
    def http_client(self) -> ABCHTTPClient:
...
class _TimeHTTPClientWrapper(ABCHTTPClient):
    ...
    def wrap_factory(cls, network_context: NetworkContext):
        ...
        factory = network_context.network.get_http_client
        functools.wraps(factory)

        def wrapped_factory():
            return cls(network_context)
    ...
    def __init__(self, network_context: NetworkContext) -> None:
        ...
        self.http_client = network_context.network.get_http_client()
    ...
    def send(self, stream, method, url, **kwargs) -> httpx.Response:
        ...
        request is recorded by the :keyword:`with` context manager
        ...
        """
        with self.network_context.record_http_time():
...
class NetworkContextRetryFunction(NetworkContext):
    ...
    def http_client(self) -> ABCHTTPClient:
        if self._http_client is None:
            self._http_client = _RetryFunctionHTTPClient(self)
        return self._http_client
...
class _RetryFunctionHTTPClient(ABCHTTPClient):
    ...
    def __init__(self, network_context: NetworkContextRetryFunction):
        self.network_context = network_context
        self.http_client = network_context._http_client_factory()
...
class NetworkContextRetrySameHTTPClient(NetworkContext):
    ...
    def http_client(self) -> ABCHTTPClient:
        if self._http_client is None:
            self._http_client = _RetrySameHTTPClient(self)
        return self._http_client
...
class _RetrySameHTTPClient(ABCHTTPClient):
    ...
    def __init__(self, network_context: NetworkContextRetrySameHTTPClient):
        self.network_context = network_context
        self.http_client = network_context._http_client_factory()
...
class NetworkContextRetryDifferentHTTPClient(NetworkContext):
    ...
    def http_client(self) -> ABCHTTPClient:
        if self._http_client is None:
            self._http_client = _RetryDifferentHTTPClient(self)
        return self._http_client
