...
class HTTPClient(Client):
    ...
    def safesearch(self) -> SafeSearchType:
        ...
        safe_search: SingleChoice = self.prefs.components["safesearch"]  # type: ignore
    ...
    def ui_locale_tag(self) -> str:
        ...
        tag = "en"
        for tag in [
                self.prefs.components.get("ui_locale_tag", ""),
    ...
    def search_locale_tag(self) -> str:
        ...
        for tag in [
                ...
                self.prefs.value("ui_locale_tag"),
    ...
    def engine_names(self) -> set[str]:
        ...
        def _engines_in_categories(c_list: list[str]) -> set[str]:
            ...
            for c in c_list:
                for eng in searx.engines.categories.get(c, []):
                    if eng.name not in self.prefs.disabled_engines:
                        e.add(eng.name)
            return e

        # get engine names from categories, !bang & form data

        categs: MultipleChoice = self.prefs.components["categories"]  # type: ignore
        ...
        for name in sxng_request.form.get("engines", "").split(","):
            name = name.strip()
            if name in searx.engines.engines and name not in self.prefs.disabled_engines:
                eng_names.add(name)
