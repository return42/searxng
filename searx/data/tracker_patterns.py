...
URLArgListType = list[tuple[str, str]]
...
class TrackerPatternsDB:
    ...
    def add(self, rule: RuleType):
        self.cache.set(
            key=rule[self.url_regexp],
            value=(
                rule[self.url_ignore],
                rule[self.del_args],
            ),
            expire=None,
        )
    ...
    def rules(self) -> Iterator[RuleType]:
        for key, value in self.cache.pairs(ctx=self.ctx_name):
            yield key, value[self.url_ignore], value[self.del_args]
    ...
    def iter_clear_list(self) -> Iterator[RuleType]:
        ...
        if resp is None:
            log.error(f"TRACKER_PATTERNS: failed fetching ClearURL rule lists")
            return

        for rule in resp.json()["providers"]:
            yield (
                rule["urlPattern"].replace("\\\\", "\\"),  # fix javascript regex syntax
                [exc.replace("\\\\", "\\") for exc in rule["exceptions"]],
                rule["rules"],
            )
    ...
    def clean_url(self, url: str) -> bool | str:
        ...
        for rule in self.rules():
            ...
            if not re.match(rule[self.url_regexp], new_url):
                # no match / ignore pattern
                continue

            do_ignore = False
            for pattern in rule[self.url_ignore]:
            ...
            query_args: URLArgListType = list(parse_qsl(parsed_new_url.query))

            for name, val in query_args.copy():
                # remove URL arguments
                for pattern in rule[self.del_args]:
                    if re.match(pattern, name):
                        log.debug("%s remove tracker arg: %s='%s'", parsed_new_url.netloc, name, val)
