...
from .components import Form, Field, SingleChoice, Bool, MultipleChoice, BoolGrp
...
def sxng_pref_list():
    return [
        ...
        EngineMapPrefs("pref", "engines", searx.engines.engines),
        SingleChoice(
            name="autocomplete",
...
class Preferences(Form):
    ...
    def process_request(self):
        ...
        # If language is defined neither in settings nor in preferences use the
        ...
        if not self.components["language"].value:
            ...
            self.components["language"].set(tag)
            logger.debug('set language %s (from browser)', tag)

        # If UI locale is defined neither in settings nor in preferences use the
        # browser Accept-Language header.
        ...
        if not self.components["ui_locale"].value:
            ...
            self.components["ui_locale"].set(tag)
            logger.debug('set locale %s (from browser)', tag)
        ...
        if "webkit" in user_agent and "android" in user_agent:
            self.components["method"].value = "GET"
            self.components["method"].lock(True)

    def validate_token(self, engine) -> bool:
        valid = True
        token_list = [t.strip() for t in self.components["tokens"].split(",")]
    ...
    def req_plugins(self) -> list[str]:
        ...
        ret_val = []
        grp = self.components["plugins"]
        for plg_id, field in grp.members.values():
            if field.value:
