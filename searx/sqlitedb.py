...
class DBSession:
    ...
    def conn(self) -> sqlite3.Connection:
        msg = f"[{threading.current_thread().name}] {self.app.__class__.__name__}({self.app.db_url})"
        if self._conn is None:
            self._conn = self.app.connect()
            logger.debug("DBSession: connected " + msg)
        else:
            logger.debug("DBSession: already connected " + msg)
