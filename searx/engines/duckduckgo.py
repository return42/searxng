...
The API is reverse-engineered from DuckDuckGo's HTML webpage
https://html.duckduckgo.com/html/ and may be subject to additional bot detection
mechanisms and breaking changes in the future.
...
- api (str): API endpoint identifier, typically "d.js" ("i.js", "v.js" and
  "news.js" for images, videos and news)
- vqd (str): Validation query digest
- kl (str): Keyboard language/region code (e.g., "en-us")
- df (str): Time filter, maps to values like "d" (day), "w" (week), "m" (month),
  "y" (year)
...
session_type: "SessionType" = "duckduckgo.com"
...
def setup(engine_settings: dict[str, t.Any]) -> bool:  # pylint: disable=unused-argument
    ...
    CACHE = EngineCache("duckduckgo")
    return True


def get_vqd(query: str) -> str | None:
    """Returns the ``vqd`` that fits to the *query* / or None."""
    key = CACHE.secret_hash(f"{query}")
    return CACHE.get(key=key, default=None)


def set_vqd(query: str, digest: str):
    key = CACHE.secret_hash(f"{query}")
    CACHE.set(key=key, value=digest, expire=3600)


def get_ddg_lang(eng_traits: EngineTraits, sxng_locale: str, default: str = "en_US"):
...
def is_ddg_captcha(dom: lxml.html.HtmlElement):
    """In case of `CAPTCHA ddg response`_ its own *not a Robot* dialog and is not
    redirected to a CAPTCHA page.

    .. _CAPTCHA ddg response: https://github.com/searxng/searxng/issues/3927
    """
...
def response(resp: "SXNG_Response") -> EngineResults:
    ...
    if is_ddg_captcha(doc):
        ...
        raise SearxEngineCaptchaException(
            suspended_time=0,
            message=f"CAPTCHA ({resp.search_params['data'].get('kl')})",
        )

    vqd_digest: str = doc.xpath("//input[@name='vqd']/@value")
    vqd_query: str = doc.xpath("//input[@name='vqd']/../input[@name='q']/@value")
    if vqd_digest and vqd_query:
        set_vqd(query=vqd_query[0], digest=vqd_digest[0])
    ...
    for div_result in eval_xpath(doc, "//div[@id='links']/div[contains(@class, 'web-result')]"):
        ...
        _t = eval_xpath(div_result, ".//h2/a")
        if not _t:
            # this is the "No results." item in the result list
            continue

        _title: str = extract_text(_t)  # pyright: ignore[reportAssignmentType]
        _url: str = eval_xpath(div_result, ".//h2/a/@href")[0]
        _content: str = extract_text(  # pyright: ignore[reportAssignmentType]
            eval_xpath_getindex(div_result, ".//a[contains(@class, 'result__snippet')]", 0, [])
        )
        results.add(results.types.MainResult(title=_title, url=_url, content=_content))
