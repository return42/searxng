...
.. note::
   ...
   :ref:`babel` and note that when searching for radio stations, users are more
   likely to search by name than by region or language
...
def response(resp):
    ...
    for result in json_resp:
        ...
        location = [y for y in [result.get(x) for x in ['country', 'state']] if y]
